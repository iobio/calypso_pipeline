...
def getVcfFiles(mosaicConfig, api_sf, projectId, mosaicSamples):
  ...
  for sample in mosaicSamples:
    ...
    # If there are more than one vcf file for the sample, fail
    if len(vcfFiles) != 1: fail('calypso_vcf_files: Mosaic has no, or multiple vcf files associated with it, so the file to use cannot be determined')
