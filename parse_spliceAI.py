...
def parseCommandLine():
  ...
  # Required arguments
  parser.add_argument('--input_tsv', '-i', required = True, metavar = "string", help = "The input SpliceAI tsv file")
...
def parseSpliceAI(args):
  global mosaicInfo

  # Open the tsv file
  try: spliceaiTsv = open(args.input_tsv, "r")
  ...
  for line in spliceaiTsv:
    ...
    if line.startswith("CHROM"): print(line, file = outFile)
      ...
      if hasValue:
        ...
        # Check if the field contains a comma. If so, the value to upload will be that with the highest value
        if "," in fields[5]: availableValues = fields[5].split(",")
        else: availableValues = [fields[5]]
        annotationLines = {}

        # Loop over the available annotations and process
        for availableValue in availableValues:
          annotations     = availableValue.split("|")
          annotationLine  = ""
          ...
          for annotation in sorted(mosaicInfo["resources"]["SpliceAI"]["annotations"]):
            ...
            # If this annotation requires an operation, perform the operation
            if "operation" in annotationInfo:

              # If the operation is to find the max, find the max value of the given positions
              if annotationInfo["operation"] == "max":
                findMaxOf = []
                for position in annotationInfo["positions"]: findMaxOf.append(annotations[int(position) - 1])
                maxValue        = max(findMaxOf)
                annotationLine += "\t" + str(max(findMaxOf))

            # Otherwise, write out the required field
            elif "position" in annotationInfo:
              position  = annotationInfo["position"]
              valueType = annotationInfo["type"]
              value     = annotations[int(position) - 1]

              # Check that the value is of the correct type
              if valueType == "float":
                try: value = float(value)
                except: fail("Incorrect type for SpliceAI annotation at position: " + fields[0] + ":" + fields[1])
              elif valueType == "int":
                try: value = int(value)
                except: fail("Incorrect type for SpliceAI annotation at position: " + fields[0] + ":" + fields[1])
              annotationLine += "\t" + str(value)

          # Store the line of annotations with the max value as the key
          annotationLines[str(maxValue)] = annotationLine

        # Write out the line
        print(outputLine, annotationLines[max(annotationLines.keys())], sep = "", file = outFile)
