...
def main():
  ...
  # Parse the spliceAI file and update
  parseAnnotation(args)
...
def parseCommandLine():
  ...
  # Required arguments
  parser.add_argument('--input_tsv', '-i', required = True, metavar = "string", help = "The input tsv file")
  parser.add_argument('--annotation', '-a', required = True, metavar = "string", help = "The annotation to be processed")
  ...
  parser.add_argument('--reference', '-r', required = False, metavar = "string", help = "The reference genome to use. Allowed values: '37', '38'")
...
# Parse the SpliceAI tsv and update ready for upload to Mosaic
def parseAnnotation(args):
  global mosaicInfo

  # Open the tsv file
  try: inputTsv = open(args.input_tsv, "r")
  ...
  # If there is a method for splitting ties, store that
  splitTie = mosaicInfo["resources"][args.annotation]["post_command"]["split_tie"] if "split_tie" in mosaicInfo["resources"][args.annotation]["post_command"] else False

  # Get the expected data type
  annotations = list(mosaicInfo["resources"][args.annotation]["annotations"].keys())
  if len(annotations) != 1: fail("This script only handles tsv files with a single annotation")
  expectedType = mosaicInfo["resources"][args.annotation]["annotations"][annotations[0]]["type"]
  ...
  for line in inputTsv:
    ...
    if line.startswith("CHROM"): print(line, file = outFile)
      ...
      if hasValue:
        ...
        # If there is comma in the annotation and no way of splitting the tie, file
        if "," in fields[5]:
          if not splitTie: fail("There is no way way to split the tie for multivalues annotations")

          # Split the annotation
          annotations = fields[5].split(",")

        # If there is no comma, just add the single value to the list
        else: annotations = [fields[5]]

        # Loop over all the values, check they are acceptable
        finalValues = []
        for value in annotations:

          # Check the value conforms to the type
          if expectedType == "float":
            try: finalValue = float(value)
            except: fail("Annotation value is not a float at position: " + fields[0] + ":" + fields[1])

            # Make sure the value falls between 1E-37 and 1E+37
            if finalValue <= 1e-37: finalValue = 1e-37
            elif finalValue >= 1e37: finalValue = 1e37
            finalValues.append(finalValue)

          else: fail("Can't handle anything other than floats")

        # If the max value is to be carried on, determine the max
        if splitTie == "max": value = max(finalValues)

        # Write out the line
        print(outputLine, "\t", value, sep = "", file = outFile)
