...
#  import api_variant_filters as api_vf
  ...
  # Loop over all Mosaic projects, ignore collections, and determine which projects have undergone the Calypso
  # annotation pipeline
  calypsoProjectIds = getCalypsoProjects(args, api_p)
...
# Loop over all Mosaic projects, ignore collections, and determine which projects have undergone the Calypso
# annotation pipeline
def getCalypsoProjects(args, api_p):

  try: data = json.loads(os.popen(api_p.getProjects(mosaicConfig)).read())
  except: fail('Couldn\'t get project ids')

  # Loop over the data
  for project in data['data']:

    # Only consider projects with the defined reference
    print(args.reference, project['reference'])
    if project['reference'] == args.reference:
      print(project)
